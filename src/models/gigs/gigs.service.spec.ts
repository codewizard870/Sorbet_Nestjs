...
describe("GigsService", () => {
  ...
  it("should find one of the gigs and return it", async () => {
    const gig = await service.findOne(newGig.id)
  ...
  it("should update a gig by id", async () => {
    const updatedGig = await service.update(newGig.id, updateGigDto)
    expect(service.update).toBeCalled()
    expect(updatedGig).toEqual(expect.any(Object))
    expect(updatedGig).toEqual(
      { message: `Successfully updated gig` }
    )
    const findUpdatedGig = await service.findOne(newGig.id)
    expect(service.findOne).toBeCalled()
  ...
  it("should remove a gig by id", async () => {
    const removedGig = await service.remove(newGig.id)
    ...
    const findDeletedGig = await service.findOne(newGig.id)
    expect(service.remove).toBeCalled()
